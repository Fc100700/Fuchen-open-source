...
class LoginWindow(QMainWindow):  # 实例化登录窗口
    ...
    def __init__(
        self, s, connect_status, stdout_stream, stderr_stream, globals_dict=None
    ):
        super().__init__()
        self.s = s
        ...
        self.ui.pushButton_tourist.clicked.connect(
            lambda: self.LOGIN("tourist_login")
        )  # 游客登录
        ...
        self.ui.Account_lineEdit.returnPressed.connect(
        ...
        self.ui.Version_Label.move(
            self.ui.Number_Label.pos().x(), self.ui.Number_Label.pos().y()
        )
            ...
            if mode == "login":
                ...
                if log_ST == "pass":
                    ...
                    QMessageBox.information(
                        self,
                        "提示",
                        "账号密码输入次数过多 账号已被锁定!请于一小时后重新登录",
                    )
                else:
                    print("密码错误 请重试")
            elif mode == "tourist_login":
                self.TypedJSONClient("login_tourist", {"position": self.city_name})
                self.ui.Login_Button.setEnabled(False)
            ...
            if log_ST == "pass":  # 密码正确
                ...
                if self.connect_status != None:
                    ...
                    if request_type == "login_successfully":
                        ...
                        self.globals["avatar_status"] = request_data.get(
                            "avatar_status"
                        )
                        self.globals["avatar_date"] = request_data.get("avatar_date")
                        self.globals["exp"] = request_data.get("exp")
                        self.globals["access_token"] = request_data.get("access_token")
                        self.globals["refresh_token"] = request_data.get(
                            "refresh_token"
                        )
                else:
                    ...
                    self.globals["access_token"] = None
                    self.globals["refresh_token"] = None
                        ...
                        with open("./temp/avatar.png", "wb") as file:
                            ...
                            while total_received < file_size:
                                ...
                                progress_percentage = round(
                                    total_received / file_size * 100, 2
                                )  # 将进度转换为百分比并保留两位小数
                                self.ui.Login_Button.setText(
                                    f"正在加载用户头像 {progress_percentage}%"
                                )
                                self.ui.Login_Button.repaint()
                        print("文件写入完成")
                ...
                if (
                    self.ui.checkBox.isChecked() and self.ui.checkBox2.isChecked()
                ):  # 记住密码 自动登录
                ...
                elif self.globals["Theme"] not in ("White", "Custom"):
                    self.globals["Theme"] = "White"
                # 准备要发送的数据
                login_data = {
                    "Account": Account,
                    ...
                    "access_token": self.globals.get("access_token"),
                    "refresh_token": self.globals.get("refresh_token"),
                    # 添加其他需要传递的数据
                }
        ...
        if self.register_window_status is False:
            ...
            if self.register_window.result_value != None:
                if self.register_window.result_value[0] == "注册成功":
                    self.ui.Account_lineEdit.setText(
                        self.register_window.result_value[1]
                    )
                    self.ui.Password_lineEdit.setText(
                        self.register_window.result_value[2]
                    )
                    self.ui.checkBox.setChecked(True)
